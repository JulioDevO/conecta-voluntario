document.addEventListener('DOMContentLoaded', () => {
    ...
    const estilosOportunidades = `
        ...
        .oportunidades-section h2 { font-size: 2.5rem; color: var(--primary-color); margin-bottom: 0.5rem; }
        .oportunidades-section p { color: #666; font-size: 1.1rem; margin-bottom: 2rem; }
        
        .busca-container { display: flex; justify-content: center; margin-bottom: 3rem; }
        #inputBusca {
            width: 100%; max-width: 600px; padding: 1rem 1.5rem;
            border: 2px solid transparent; border-radius: 50px;
            box-shadow: 0 4px 15px rgba(0,0,0,0.08); font-size: 1.1rem;
            transition: all 0.3s ease;
        }
        #inputBusca:focus { border-color: var(--primary-color); box-shadow: 0 4px 20px rgba(62, 126, 164, 0.25); outline: none; }
        ...
        .vaga-card:hover { transform: translateY(-8px); box-shadow: 0 12px 24px rgba(0,0,0,0.12); }
        .vaga-card h3 { font-size: 1.4rem; color: var(--text-dark); margin-bottom: 1rem; line-height: 1.3; }
        .vaga-card p { color: #555; margin-bottom: 0.8rem; font-size: 1rem; }
        
        .vaga-nao-encontrada { color: #d9534f; font-size: 1.2rem; font-weight: bold; grid-column: 1 / -1; margin-top: 2rem; }
        
        .badge-area {
            display: inline-block; background-color: #eaf3f8; color: var(--primary-color);
